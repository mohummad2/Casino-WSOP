...
#region allInFunctions
  ...
  void Awake() {
		BBGC = GetComponent<BBGameController>();
			finaltext = transform.Find("FinalText").GetComponent<Text>();
  }
	...
	IEnumerator showFinalPlayersResult() {
		...
		for(int x = 0;x < playersCVal.Count;x++) {
				...
				if (playersCVal[x].won)
				{
					...
					finaltext.text = ("Dealer: " + BBGC.playerDataList[x].T_playerName.text + " wins " + BBStaticData.getMoneyValue(playersCVal[x].splittedValue) + " with a " + GetComponent<BBShowDownResultController>().getFormattedBestFive(playersCVal[x].bestFive));
					Debug.Log("Dealer: " + BBGC.playerDataList[x].T_playerName.text + " wins " + BBStaticData.getMoneyValue(playersCVal[x].splittedValue) + " with a " + GetComponent<BBShowDownResultController>().getFormattedBestFive(playersCVal[x].bestFive));
					wonCounter++;
					GetComponent<BBShowDownResultController>().playerFinalResultUIList[x].transform.Find("TextMoneyResult").GetComponent<Text>().text = BBStaticData.getMoneyValue(playersCVal[x].splittedValue);
				}
			       // else {
